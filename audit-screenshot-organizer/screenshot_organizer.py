...
class MiniToolbar:
    def __init__(self, app):
        ...
        outer = tk.Frame(
            ...
            highlightbackground=TOOLBAR_BORDER,
        )
    ...
    def _run_screencapture(self, args, out_path):
        try:
            ...
            if stderr:
                ...
                self.app.ui_log("Capture failed. (macOS permission is the most common cause.)")
    ...
    def capture_region(self):
        ...
        if platform.system() != "Darwin":
            self.app.ui_log("Region capture in the toolbar is macOS-native only.")
    ...
    def capture_full(self):
        ...
        def do_full():
            try:
                ...
                self.app.ui_log(f"Full capture error: {e}")
                self.app.root.after(0, lambda: self.set_status("Failed"))
...
class ScreenshotOrganizerApp:
    ...
    def build_gui(self, outer):
        ...
        ttk.Radiobutton(mode_frame, text="Playwright (Auto capture)", variable=self.mode, value="playwright", command=self.on_mode_change).pack(side="left", padx=(0, 14))
    ...
    def run_playwright_capture(self):
        try:
            ...
            with sync_playwright() as p:
                ...
                if selector:
                    el_path = ensure_unique_path(os.path.join(self.active_session_folder, f"{self.timestamp()}_element_{self.safe_name(selector)}.png"))
